...
支持多尺度输出
"""
class ViT3DWithIntermediateOutputs(nn.Module):
    def __init__(
        self,
        img_size: int = 256,
        patch_size: int = 16,
        ...
        if use_abs_pos:
            ...
            self.pos_embed = nn.Parameter(
                torch.zeros(1, grid_size, grid_size, grid_size, embed_dim)
            )
        
        # 正确构建Block3D列表（不传递depth参数）
        self.blocks = nn.ModuleList()
        for i in range(depth):  # 使用depth控制循环次数
            block = Block3D(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
