"""
为了减少训练的参数量，调整了VNet网络的相关内容
相比于V4设置卷积核大小为222而不是221，通道方面也进行调整
"""
...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        ...
        self.block_four_dw = DownsamplingConvBlock(n_filters * 8, n_filters * 16, stride=(2,2,2), normalization=normalization) # 使用221能够减少一半的flops
        ...
        self.logits_head = nn.Conv3d(n_filters, n_classes, 1, padding=0)

        # Variance head (新增不确定性头)
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        logits, variance = self.decoder(features)  # 接收双输出
        
        if turnoff_drop:
            self.has_dropout = has_dropout
            
        return logits, variance, features  # 返回元组 (logits, variance, features)
...
if __name__ == '__main__':
    ...
    model = VNet(n_channels=1, n_classes=2, normalization='batchnorm', has_dropout=True,n_filters=16)
