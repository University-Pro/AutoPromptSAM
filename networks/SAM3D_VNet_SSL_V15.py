...
v15 dev
在V14的基础上修改了一些参数
然后在训练的过程中使用了相互伪标签的方法
尝试优化一下网络计算的效率
并且看看能不能适配其他的数据集（Amos）
"""
...
class DecoupledUncertaintyPrompt(nn.Module):
    ...
    def __init__(self, n_channels: int, n_classes: int, normalization: Optional[str] = 'batchnorm',
                 ...
                 num_mc_samples=10, w_epistemic: float = 0.5, w_aleatoric: float = 0.5, k_per_class=20,
    ...
    def forward(self, x):
        ...
        # 返回张量（不再是列表）
        all_coords = torch.empty((batch_size, self.k_per_class * self.num_classes, 3), 
                                dtype=torch.float32, device=device)
        all_labels = torch.empty((batch_size, self.k_per_class * self.num_classes), 
        ...
        for i in range(batch_size):
            # 获取当前样本的score_map [D, H, W]
            score_map_i = prompt_suitability_score[i].squeeze(0)  # [D, H, W]
            
            # 获取当前样本的分割概率 [C, H, W, D]
            seg_probs_i = final_segmentation_probs[i].permute(0, 2, 3, 1)  # [C, H, W, D]
            
            # 使用非极大值抑制为每个类别找到Top-K个提示点
            class_prompts = find_top_k_prompts_per_class(
                score_map=score_map_i,
                segmentation_probs=seg_probs_i,
            ...
            for class_id in range(self.num_classes):
                ...
                for prompt in prompts:
                    ...
                    sample_labels.append(prompt['predicted_class'])
            
            # 填充到固定长度
            num_points = len(sample_coords)
            max_points = self.k_per_class * self.num_classes
            
            if num_points > max_points:
                # 截断多余的点
                sample_coords = sample_coords[:max_points]
                sample_labels = sample_labels[:max_points]
            elif num_points < max_points:
                # 用最后一个点填充
                last_coord = sample_coords[-1] if sample_coords else [0, 0, 0]
                last_label = sample_labels[-1] if sample_labels else 0
                sample_coords += [last_coord] * (max_points - num_points)
                sample_labels += [last_label] * (max_points - num_points)
            
            # 转换为张量
            all_coords[i] = torch.tensor(sample_coords, dtype=torch.float32, device=device)
            all_labels[i] = torch.tensor(sample_labels, dtype=torch.long, device=device)
...
class PromptGenerator_Encoder(nn.Module):
    ...
    def __init__(
        ...
        # 检查threshold和num_points_per_class参数
    ...
    def forward(
        ...
        n_pts = self.num_points_per_class # 每个类别采样的点数
        thr = self.threshold # 概率阈值
...
def PromptGenerator_test():
    ...
    model = PromptGenerator_Encoder(
        n_channels=input_shape[1],        # 输入通道数
        n_classes=n_classes,              # 输出类别数
        normalization=normalization_type, # 修正：使用有效归一化类型
        has_dropout=False,                
...
def networktest():
    ...
    # 实例化网络
    model = Network(in_channels=1,encoder_depth=4).to(device=device)
