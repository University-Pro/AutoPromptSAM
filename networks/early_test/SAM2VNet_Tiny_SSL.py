...
class SAM2VNet(nn.Module):
    ...
    def __init__(self, checkpoint_path=None, model_cfg=None, device="cuda", output_channel=None, sam_pretrained_path=None):
        ...
        # ------------------ 加载SAM预训练权重 ------------------
        if sam_pretrained_path:  # 新增预训练加载逻辑
            self.load_sam_pretrained(sam_pretrained_path, device)
    ...
    def load_sam_pretrained(self, pretrained_path, device):
        """
        加载SAM部分的预训练权重
        策略：只加载encoder和decoder部分，跳过VNet和输入适配层
        """
        try:
            # 加载完整预训练权重
            ...
            for k, v in pretrained_dict.items():
                if any(key in k for key in sam_keys):
                    # 处理输入适配层的特殊匹配
                    if 'patch_embed.proj' in k:
                        # 跳过输入层（因为输入通道数不同）
                        # print(f"Skipping {k} due to input channel mismatch")
                        continue
                    load_dict[k] = v
            
            # 加载权重（允许部分匹配）
            self.load_state_dict(load_dict, strict=False)
            # print(f"成功加载SAM预训练权重：{len(load_dict)}/{len(pretrained_dict)} 参数已加载")
...
if __name__ == "__main__":
    ...
    # 创建Tiny SAM2模型进行尝试
    sam2_checkpoint = "./sam2_configs/sam2.1_hiera_tiny.pt"
    ...
    # 检查是否被冻结
    frozen_params = [p for p in model.encoder.parameters() if not p.requires_grad]
    print(f"Encoder被冻结的参数数量: {len(frozen_params)}")

    # 测试前向传播
    dummy_input = torch.randn(1, 1, 112, 112, 80).to(device)  # 3D输入
    # dummy_input = torch.randn(1, 1, 128, 128, 128).to(device)  # 3D输入
