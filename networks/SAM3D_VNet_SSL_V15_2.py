...
v15-2
优化网络结构
尝试大幅度减少网络的计算复杂度和参数数量
vnet的n_filters从32改为16
vnet采样数设置为3，并且添加预训练的pth
针对SAM添加更多的ImageEncoder部分，增强特征提取能力
"""
...
class ImageEncoderViT3D(nn.Module):
    def __init__(
        ...
        self.stage1_blocks = nn.ModuleList([
            Block3D(
                dim=96,
                num_heads=num_heads,
                input_size=(img_size[0]//2, img_size[1]//2, img_size[2]//2),
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                ...
                window_size=4
            ) for _ in range(2)  # 创建2个独立的Block3D实例
        ])
        ...
        self.stage2_blocks = nn.ModuleList([
            Block3D(
                dim=192, 
                num_heads=num_heads,
                input_size=(img_size[0]//4, img_size[1]//4, img_size[2]//4),
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                norm_layer=norm_layer,
                use_rel_pos=use_rel_pos,
        ...
        self.stage3_blocks = nn.ModuleList([
            Block3D(
                dim=384, 
                num_heads=num_heads,
                input_size=(img_size[0]//8, img_size[1]//8, img_size[2]//8),
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                norm_layer=norm_layer,
                use_rel_pos=use_rel_pos,
                rel_pos_zero_init=rel_pos_zero_init,
                window_size=4
            ) for _ in range(4)  # 创建4个独立的Block3D实例
        ])
        ...
        self.stage4_blocks = nn.ModuleList([
            Block3D(
                dim=768,
                num_heads=num_heads,
                input_size=(img_size[0]//16, img_size[1]//16, img_size[2]//16),
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                norm_layer=norm_layer,
                use_rel_pos=use_rel_pos,
        ...
        # patchmerging方式
        self.patchmerging0 = PatchMerging3D(input_dim=96, out_dim=192)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Patch Embedding
        # print(f'before patch embedding, input shape is {x.shape}')
        x = self.patch_embed1(x)
        # print(f'after patch_embedding, input shape is {x.shape}')
        
        # 添加位置编码
        if self.pos_embed is not None:
            x = x + self.pos_embed
            # print(f'after pos_embed, input shape is {x.shape}')
        
        # Stage 1: 2个Block
        for block in self.stage1_blocks:
            x = block(x)
        # print(f'after stage1_blocks, input shape is {x.shape}')
        
        # PatchMerging 操作
        x = self.patchmerging0(x)
        # print(f'after patchmerging0, input shape is {x.shape}')
        
        # Stage 2: 2个Block
        for block in self.stage2_blocks:
            x = block(x)
        # print(f'after stage2_blocks, input shape is {x.shape}')
        
        # PatchMerging 操作
        x = self.patchmerging1(x)
        # print(f'after patchmerging1, input shape is {x.shape}')
        
        # Stage 3: 4个Block
        for block in self.stage3_blocks:
            x = block(x)
        # print(f'after stage3_blocks, input shape is {x.shape}')
            
        # 输出结果调整维度
        x = x.permute(0, 4, 1, 2, 3)  # [B, D, H, W, C] -> [B, C, D, H, W]
        return x

class CARF_back(nn.Module):
...
class CARF(nn.Module):
    def __init__(self, dim_a, dim_b, common_dim=256):
        ...
        # 融合卷积层，输入通道为拼接后的 common_dim * 2
        self.fusion_conv = nn.Sequential(
            nn.Conv3d(common_dim * 2, common_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm3d(common_dim),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, feature_a, feature_b):
        ...
        # 将输入特征转换为 PyTorch 标准的 (B, C, D, H, W) 格式
        f_a_std = feature_a.permute(0, 1, 4, 2, 3) 
        f_b_std = feature_b.permute(0, 1, 4, 2, 3)
        ...
        # 沿通道维度拼接“共性”特征与“互补”特征
        fused_feature = torch.cat([f_a_refined, f_b_refined], dim=1)
        
        # 通过卷积层进一步处理融合后的特征
        fused_feature = self.fusion_conv(fused_feature)

        # 调整维度顺序回原始格式 (B, C, H, W, D)
        fused_feature = fused_feature.permute(0, 1, 3, 4, 2)
...
class Network(nn.Module):
    def __init__(
            self,
            in_channels: int = 1,
            image_size: Tuple[int, int, int] = (112, 112, 80), # LA数据集的大小
            # image_size: Tuple[int, int, int] = (160, 160, 80),  # Amos数据集的输入大小
            patch_size: int = 8,
            embed_dim: int = 768,
            out_chans: int = 768,
            # num_classes: int = 2,
            # num_classes: int = 16, # Amos数据集
            num_classes: int = 2, # LA数据集
        ...
        self.vnet = VNet(n_channels=self.in_channels,
                         ...
                         n_filters=16,# 32修改成16，能够减少大量的参数量
        ...
        # multi encoder merge
        self.carf_module = CARF(dim_a=128, dim_b=384, common_dim=768)
    ...
    def forward(self, x):
        ...
        # print(f'encoder feature shape is {encoder_feature[0].shape}')
        # print(f'encoder feature shape is {encoder_feature[1].shape}')
        # print(f'encoder feature shape is {encoder_feature[2].shape}')
...
def networktest():
    ...
    # input_tensor = torch.randn(1,1,128,128,128).to(device=device) # BTCV数据集的输入大小
