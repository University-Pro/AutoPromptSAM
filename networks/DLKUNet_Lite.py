...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, use_LKA=True):
        ...
        self.down2 = Down(192, 384)  # Retain two down layers

        # Removed down3 and down4 layers for a simpler network

        self.up1 = Up(384, 192)  # Corresponding upsampling layer
        self.up2 = Up(192, 96)   # Corresponding upsampling layer

        # Removed up3 and up4 layers for a simpler network
    ...
    def forward(self, x):
        ...
        # No further downsampling; start upsampling directly
        x = self.up1(x3, x2)
        x = self.up2(x, x1)
...
if __name__ == "__main__":
    ...
    model = UNet(n_channels=1, n_classes=4, use_LKA=False).to(device)
