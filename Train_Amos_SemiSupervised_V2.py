...
class TwoStreamBatchSampler_LA(Sampler):
    ...
    def __iter__(self):
        # 注意：这里的 iterate_once_LA 和 iterate_eternally_LA 需要确保在DDP环境下正确工作
        # 如果它们内部有随机操作，请确保每个进程的随机种子不同
        primary_iter = iter(self.primary_indices) # 直接迭代分布式采样器给出的索引
        secondary_iter = iter(self.secondary_indices) # 同上
        
        # 这里的 grouper_LA 需要能够正确处理迭代器
        # 为简化，我们直接在__iter__中实现批处理逻辑
...
if __name__ == "__main__":
    ...
    full_dataset = AmosDataset(
        split='train',
        config=AmosConfig(patch_size=patch_size, num_classes=args.num_classes),
        transform=train_transform,
        preload=True,  # 预加载数据到内存
        training_num=args.training_label_num + args.training_unlabel_num  # 总样本数量
    )
    logging.info(f'数据集加载完成，样本总数: {len(full_dataset)}')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            # print(f'A_output shape: {A_output.shape}, B_output shape: {B_output.shape}')

            # 监督损失（仅计算有标签数据）
            supervised_loss1 = criterion_dice(A_output[:labeled_bs], labels[:labeled_bs])
            supervised_loss2 = criterion_dice(B_output[:labeled_bs], labels[:labeled_bs])
            # print(f'Supervised Loss: {supervised_loss.item()}')
            
            # print(f'a_output[labeled_bs:] shape: {A_output[labeled_bs:].shape}, B_output[labeled_bs:] shape: {B_output[labeled_bs:].shape}')
            # 一致性损失（无标签数据，A指导B）
            ...
            # total_loss = supervised_loss1+ supervised_loss2 + args.consistency_weight * consistency_loss
            total_loss = supervised_loss2 + args.consistency_weight * consistency_loss
            ...
            epoch_dice_loss1 += supervised_loss1.item()
            epoch_dice_loss2 += supervised_loss2.item()
            epoch_dice_loss += (supervised_loss1.item() + supervised_loss2.item())
